namespace Krafteq.Akamai.NetStorage
{
    ...
    static class StreamUtil
    {
        ...
        {
            return await ReadFullyAsync(input, DefaultBufferSize);
        }
        ...
        public static async Task<byte[]> ReadFullyAsync(Stream input, int bufferSize)
        {
            ...
            return await ReadFullyAsync(input, new byte[bufferSize]);
        }
        ...
        public static async Task<byte[]> ReadFullyAsync(Stream input, byte[] buffer)
        {
            ...
            using (MemoryStream tempStream = new MemoryStream())
            {
                await CopyAsync(input, tempStream, buffer);
                // No need to copy the buffer if it's the right size
                if (tempStream.Length == tempStream.GetBuffer().Length)
                {
                    return tempStream.GetBuffer();
        ...
        {
            await CopyAsync(input, output, DefaultBufferSize);
        }
        ...
        public static async Task CopyAsync(Stream input, Stream output, int bufferSize)
        {
            ...
            await CopyAsync(input, output, new byte[bufferSize]);
        }
        ...
        {
            return await ReadExactlyAsync(input, new byte[bytesToRead]);
        }
        ...
        {
            return await ReadExactlyAsync(input, buffer, buffer.Length);
        }
        ...
        {
            return await ReadExactlyAsync(input, buffer, 0, bytesToRead);
        }
        ...
        public static async Task<byte[]> ReadExactlyAsync(Stream input, byte[] buffer, int startIndex, int bytesToRead)
        {
            ...
            while (index < bytesToRead)
            {
                int read = await input.ReadAsync(buffer, startIndex + index, bytesToRead - index);
                if (read == 0)
                {
                    throw new EndOfStreamException
                        (String.Format("End of stream reached with {0} byte{1} left to read.",
